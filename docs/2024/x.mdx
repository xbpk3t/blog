...
- 在对应项目的 PR 页面里，最好标明对应的 issue，以及注意，随便查查，稍有要求的 coder 都会吐槽不遵守规范的人。
...
[通过分析gin、beego源码，读懂web框架对http请求处理流程的本质](https://mp.weixin.qq.com/s?__biz=MzAxMTA4Njc0OQ==&mid=2651453917&idx=1&sn=334020f003deec68f268d1c36143ddca)
...
这批人就是吃饱了撑的，做 SEO 为什么偏要折腾这些 SPA 时代的框架而不是去做模板引擎渲染，可偏偏他们做成了，然后成品大家都觉得好用。
...
1. **CMD 指令**：
   - 它定义了容器启动时默认执行的命令。
   - 用户在运行容器时可以通过命令行替换这个默认命令。

2. **ENTRYPOINT 指令**：
   - 它定义了容器启动时必须执行的主命令。
   - 用户在运行容器时可以通过命令行向这个主命令添加参数，但不能替换主命令本身。

**区别**：
- `CMD` 可以被用户完全替换，包括命令和参数。
- `ENTRYPOINT` 只能接受用户添加的参数，不能被替换。

**例子**：
假设我们有一个 Dockerfile，里面定义了如下指令：
...
- 对于使用 `CMD` 的容器，用户可以通过命令行完全替换默认命令：
  docker run myimage ls -l
  这里，`echo "Hello World"` 被 `ls -l` 替换了。

- 对于使用 `ENTRYPOINT` 的容器，用户只能添加参数，不能替换 `echo`：
  docker run myimage "Hello Kimi"
  这里，`echo` 后面添加了参数 `"Hello Kimi"`，但 `echo` 命令本身没有被替换。

希望这次解释更清晰了！如果还有疑问，随时问我。
...
[B树、B+树索引算法原理（上） - codedump的网络日志](https://www.codedump.info/post/20200609-btree-1/#%E6%83%85%E5%86%B53-b%E4%B8%8E%E5%85%84%E5%BC%9F%E8%8A%82%E7%82%B9%E5%90%88%E5%B9%B6%E6%95%B0%E6%8D%AE)
