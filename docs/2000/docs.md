...
反思了一下，PQ4R本身已经高度浓缩了整个“从输入到输出”的通路
...
我仍然记得n年前第一次使用OneTab的感受，一种非常放松，如释重负的感觉。之后也尝试了各种类似工具，但是还没找到其他更好的替代品。这足以说明OneTab的优秀。那么，为什么要删掉这个插件呢？

用来解决什么问题？

用OneTab也属于某种“虚假的掌控感”，往往很多网页看都不看（实际上顺手就能处理掉的），直接往OneTab中一塞，反而导致效率很低。

就是我之前说的“就那几个网页来来回回，从bm放到onetab，再从onetab弄到bm。写到docs里，再打开，再重复一遍”
...
有两个问题，一个是，qs本身是否掌握了，另一个就是，是否记得已经记录过这个qs。二者缺一不可，否则都会出现重复
...
换个角度来看，现在用这套记忆方法，或者说学习方法，相比于之前写笔记，或者说学习记录 心得，的方法来说。上周有天整理了下中学时的必背古诗词，发现之前背的滚瓜烂熟的这些东西，只有几首还能整篇背下来，大部分也就还能背上一两句。必备诗词且如此，更别说自己摘抄的那些古诗词了。很怅然，之前花了大功夫，摘抄每天早读都要反复背诵的东西，现在竟然就都不记得了。

当然，细究起来，二者肯定是不同的。背诵古诗词的核心是想象，想象古诗构建起来的那个场景。代码相关的东西则不然


要复杂很多，需要逻辑，需要一定的元技能和 kernel algo，面向需求，面向使用和使用场景
...
[提问的艺术 (豆瓣)](https://book.douban.com/subject/25806793/)
...
回去把那几份github上的面试题删掉，我有qs的话，是不需要这个的

应该给qs立规矩（或者说规范），比如说

- 某种“开闭原则”：不轻易删除
- qs要添加需求（或者说场景）。举个例子，之前在retry-go里有个qs是“怎么获取当前attempts”，应该改为“需要在log中打印当前重试次数，怎么获取当前attempts”
- 好的qs，应该可以立即根据这些qs写篇文章，骨架和内容俱在，也不需要任何润色。逻辑是清晰的。针对这点，我想到我可能需要
- ***qs内容本身应该参考STARR原则（也就是 situation, task, action, result, review），也就是说上面说的“qs需要场景，否则就毫无意义”***
